...
def calibrate_gauge(filename):
	...
	cv2.imwrite(filename + "-calibration.jpg", img)
...
def get_current_value(img, min_angle, max_angle, min_value, max_value, x, y, r, filename):
	...
	cv2.imwrite(filename + "-needle.jpg", img)
...
def main():	
	...
	min_angle, max_angle, min_value, max_value, units, x, y, r = calibrate_gauge(filename)

    #feed an image (or frame) to get the current value, based on the calibration, by default uses same image as calibration
	img = cv2.imread(filename + ".jpg")
	val = get_current_value(img, min_angle, max_angle, min_value, max_value, x, y, r, filename)
